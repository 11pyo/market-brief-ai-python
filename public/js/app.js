...
window.addEventListener('appinstalled', () => {
  // 이미 설치됨 — 모든 프로모 숨김
  ['header', 'bottom', 'sidebar'].forEach(id => _hidePwaSlot(id));
...
function initPwaPromo() {
  ...
  const dismissed = localStorage.getItem('pwa-promo-dismissed') === '1';
  if (isStandalone || dismissed) {
    ['header', 'bottom', 'sidebar'].forEach(id => _hidePwaSlot(id));
  }
}

function _hidePwaSlot(id) {
  const slot = document.getElementById('pwa-slot-' + id);
  if (slot) slot.style.display = 'none';
}

function dismissPwaPromo(id) {
  _hidePwaSlot(id);
  // 하나라도 닫으면 세 곳 모두 닫고 영구 기억
  ['header', 'bottom', 'sidebar'].forEach(k => _hidePwaSlot(k));
  localStorage.setItem('pwa-promo-dismissed', '1');
}
...
function triggerPwaInstall() {
  if (_pwaPrompt) {
    ...
    showToast('이미 홈 화면에 추가되어 있습니다!', 'info');
  } else {
    showToast('브라우저 주소창의 설치(⊕) 아이콘을 눌러주세요.', 'info');
  }
}

function _showIosGuide() {
  let el = document.getElementById('pwa-ios-guide');
  if (!el) {
    el = document.createElement('div');
    el.id = 'pwa-ios-guide';
    el.className = 'pwa-ios-guide';
    el.innerHTML = `
      <div class="pwa-ios-guide-title">📲 홈 화면에 추가하는 방법</div>
      <div class="pwa-ios-guide-steps">
        1. 하단 <strong>공유 버튼</strong> ⬆️ 탭<br>
        2. <strong>"홈 화면에 추가"</strong> 선택<br>
        3. 오른쪽 상단 <strong>"추가"</strong> 탭
      </div>
      <button class="pwa-ios-guide-close" onclick="this.closest('.pwa-ios-guide').classList.remove('show')">닫기</button>
    `;
